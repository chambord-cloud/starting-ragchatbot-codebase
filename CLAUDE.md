...
This is a full-stack RAG chatbot that answers questions about course materials using semantic search over ChromaDB and Claude for response generation, served as a single FastAPI process (API + static frontend on port 8000).
...
3. **Query flow** (`backend/rag_system.py`): User query → AI generator creates a prompt → Claude decides whether to invoke the `search_course_content` tool → tool searches ChromaDB with optional course name and lesson filters → results fed back to Claude → final answer with sources.
4. **AI generation** (`backend/ai_generator.py`): Uses Anthropic Claude with tool-use support. System prompt instructs Claude to search only for course-specific questions, answer general knowledge directly, and avoid meta-commentary. Temperature 0, max 800 tokens.
...
- `backend/config.py` — Single `Config` dataclass with all settings. Loads `.env` via python-dotenv. Key values: `ANTHROPIC_MODEL`, `CHUNK_SIZE` (800), `CHUNK_OVERLAP` (100), `MAX_RESULTS` (5), `MAX_HISTORY` (2).
...
- Claude decides **when** to search (via tool-choice), not the application — the system prompt is the control surface for this behavior.
